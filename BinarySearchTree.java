...
public class BinarySearchTree<K extends Comparable<K>,V> {
    ...
    private int size;
    private BSTNode<K,V> deletedNode=null;
    ...
    private BSTNode<K,V> deleteAux(BSTNode<K,V> localRoot, BSTNode<K,V> parent, K key) {
         ...
         */
        root=deleteNode(localRoot,key);
        updateHeight(localRoot);
        updateSize(localRoot);

        return deletedNode;
    }
    public BSTNode<K,V> deleteNode(BSTNode<K,V> root, K key){
        if(root==null){
            return null;
        }
        if(key.compareTo(root.key)<0){
            ...
            if(root.right==null){
                ...
                BSTNode<K,V> rootPointer=root.right;
                while(rootPointer.left!=null){
                    rootPointer=rootPointer.left;
                }
                rootPointer.left=root.left;
                return root.right;
    ...
    public int updateHeight(BSTNode<K,V> root){
        if(root==null){
            return 0;
        }
    ...
    public int updateSize(BSTNode<K,V> root){
        if(root==null){
            return 0;
        }
        int left=updateSize(root.left);
        int right=updateSize(root.right);
        root.size=1+left+right;
        return 1+left+right;
    }
    ...
    public Map<Integer, List<BSTNode<K,V>>> getNodeLevels() {
        ...
        int index=0;
        List<BSTNode<K,V>> parentList= new ArrayList();
        parentList.add(this.root);
        nodeLevels.put(index++,parentList);
        while(!parentList.isEmpty()){
            List<BSTNode<K,V>> childList= new ArrayList();
            for(BSTNode<K,V> node: parentList){
                if(node.left!=null){
            ...
            if(!childList.isEmpty()) {
                nodeLevels.put(index++, childList);
            }
    ...
    public List<BSTNode<K,V>> getRange(K key1, K key2) {
         ...
         */
        //COMMENT: CALLS RECURSIVE FUNCTION WHICH DOES A POSTORDER TRAVERSAL TO GET A LEFT AND RIGHT LIST OF VALUES THAT ARE WITHIN THE RANGE.
        //THE LEFT AND RIGHT LISTS ARE MERGED TOGETHER AND SORTED AND RETURNED.

        return  DFS(this.root,key1,key2);


    }
    public List<BSTNode<K,V>> DFS(BSTNode<K,V> root,K key1,K key2){
        if(root==null){
            return new ArrayList();
        }
        List<BSTNode<K,V>> left=DFS(root.left,key1,key2);
        List<BSTNode<K,V>> right=DFS(root.right,key1,key2);
        left.addAll(right);

        int result1=root.key.compareTo(key1);
        int result2=root.key.compareTo(key2);
        if((result1>=0)&&(result2<=0)){
            left.add(root);
        }
        Collections.sort(left,(a,b)->a.key.compareTo(b.key));
        return left;
    ...
    public int rank(BSTNode<K,V> root, K key){
        if(root==null){
            return 0;
        }
        int left=0;
        ...
        if(root.key.compareTo(key)<0){
            left=(root.left==null)?0:root.left.size;
            right=rank(root.right,key);
            return 1+left+right;
        }
        else if(root.key.compareTo(key)>0){
            right=0;
            left=rank(root.left,key);
            return left+right;
        }
        else {
            right=0;
